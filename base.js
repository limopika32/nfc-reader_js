...
const serviceCode = Uint8Array.of(0xff, 0xff);

const device = await navigator.usb.requestDevice({
    "filters": [{"vendorId": 0x054C}]
});

let devConfigValue = device.configuration.configurationValue;
let devIO = device.configuration.interfaces[0].interfaceNumber;
let endPointIn = device.configuration.interfaces[0].alternate.endpoints.find(element => element.direction=="in").endpointNumber;
let endPointOut = device.configuration.interfaces[0].alternate.endpoints.find(element => element.direction=="out").endpointNumber;
...
async function initialTransmit(){
...
function getStid(rawArray){
    let stid = "";

    // 7 digit, binaly of ascii(utf-8)
    rawArray.slice(29, 36).forEach(
        element => stid += String.fromCharCode(element)
    );
...
let rawdata = await readWithoutEncryption(idm, 1, serviceCode);
